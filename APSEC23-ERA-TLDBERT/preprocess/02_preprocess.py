...
def preprocess(file_path: str, out_dir: str):
    
    issue_ct = 0
    file_pd = pd.read_csv(file_path, sep=';', encoding='utf-8', low_memory=False)
    # file_pd = file_pd.assign(text=[' '] * file_pd.shape[0])
    ...
    for idx, row in file_pd.iterrows():
        try:
            ...
            # row['text'] = row['title'] + '. ' + row['description'] + '. ' + row['comments']
            issue_ct += 1
    ...
    file_pd.to_csv(out_dir + '/cleaned_issues/' + filename, sep=';', encoding='utf-8', index=False)
